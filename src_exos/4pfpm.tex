...
\begin{tasks}(1)
\task $\displaystyle (x^{2}-3)'$ Utilisons la formule $(u+v)' = u' + v'$ et $(x^n)' = nx^{n-1}$ :
\begin{align*}
(x^{2}-3)' &= (x^{2})' - (3)'\\
&= 2x^{2-1} - 0\\
&= \boxed{2x}
...
\task $\displaystyle \left(\dfrac{2}{x^{5}}\right)'$ Réécrivons d'abord avec un exposant négatif : $\dfrac{2}{x^{5}} = 2x^{-5}$.
...
\task $\displaystyle \left(\sqrt{2x^{3}-3}\right)'$ Réécrivons avec un exposant fractionnaire : $\sqrt{2x^{3}-3} = (2x^{3}-3)^{1/2}$.
...
\task $\displaystyle \left(\sqrt[3]{x^{3}+1}\right)'$ Réécrivons avec un exposant fractionnaire : $\sqrt[3]{x^{3}+1} = (x^{3}+1)^{1/3}$.
...
\[\boxed{\left(\dfrac{x}{\sqrt[3]{x^{3}+1}}\right)^2}\]
\end{tasks}
