\titre{}
\theme{calcLit}
\auteur{Nathan Scheinmann}
\niveau{1M}
\source{musy}
\type{serie}
\piments{2}
\pts{}
\annee{2425}

\contenu{
\tcblower
On considère le nombre $123456789^2-123456786 \cdot 123456792$.
\begin{tasks}(2)
\task Calculer ce nombre à l'aide d'une calculatrice.
\task Poser $x=123456789$ et exprimer le nombre considéré en fonction de $x$.
\task Développer et réduire l'expression trouvée en b).
\task Que conclure des calculs précédents ?
\end{tasks}	
}
\correction{
\tcblower
\begin{tasks}(2)
	\task Calculer. Elle devrait donner $0$.
	\task $x^2-(x-3)(x+3)$
	\task $9$
	\task La calculatrice se trompe à cause d'une erreur d'arrondi. Dans ce cas, la factorisation permet de calculer rapidement et correctement. 
\end{tasks}
}

